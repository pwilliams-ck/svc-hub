...
func buildAdminClient(url string, proxy func(*http.Request) (*url.URL, error)) *Client {
	...
	base := NewDuoApi(ikey, skey, host, userAgent, SetTimeout(1*time.Second), SetInsecure(), SetProxy(proxy))
	return New(*base)
}
...
func TestCreateAccount(t *testing.T) {
	ts := httptest.NewTLSServer(
		http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			fmt.Fprintln(w, createAccountResponse)
		}),
	)
	defer ts.Close()
	...
	accountToCreate := Account{
		Name: "testing boii account",
	}

	result, err := duo.createAccount(accountToCreate)
	if err != nil {
		t.Errorf("Unexpected error from createAccount call %v", err.Error())
	}
	if result.Stat != "OK" {
		t.Errorf("Expected OK, but got %s", result.Stat)
	}
	if result.Response.Name != accountToCreate.Name {
		t.Errorf("Expected Username to be %s, but got %s", accountToCreate.Name, result.Response.Name)
	}
}
